...
The status line is a custom status line for Claude Code sessions, displaying project context, costs, and token metrics.
...
│                         CLAUDE CODE RUNTIME                         │
│                                                                     │
│  settings.json ──► statusLine.command ──► scripts/statusline.sh    │
│      (config)                                (renderer)             │
└─────────────────────────────────────────────────────────────────────┘
                                │
                                │ JSON stdin
...
|------|------|------------|
| `scripts/statusline.sh` | Render compact status line | Claude Code (via settings.json) |
...
| `settings.json` statusLine config | Registration | Claude Code startup |
...
Token data is expensive to compute (parses JSONL). The status line:
...
This keeps the status line responsive while data updates in the background.
...
### Status line shows no tokens

1. Check session file exists: `ls ~/.claude/projects/*/*.jsonl`
2. Verify get-session-tokens.sh works: `~/.claude/skills/status-line-live/scripts/get-session-tokens.sh <session_id>`
...
2. Check statusline.sh is executable: `chmod +x ~/.claude/skills/status-line-live/scripts/statusline.sh`
3. Test manually: `echo '{"workspace":{"current_dir":"."}}' | ~/.claude/skills/status-line-live/scripts/statusline.sh`

## Related Files

- `~/.claude/README.md` - Quick reference for status line format
