...
```bash
for d in $(find backlog -mindepth 1 -maxdepth 1 -type d ! -name todo ! -name done ! -name failed); do
  for f in "$d"/*.md; do
    [[ -f "$f" ]] || continue
...
in_flight=$(find backlog -mindepth 2 -maxdepth 2 -name '*.md' -type f ! -path 'backlog/todo/*' ! -path 'backlog/done/*' ! -path 'backlog/failed/*' | wc -l | tr -d ' ')
